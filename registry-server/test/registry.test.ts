...
test("query_reputation is symmetric: reviews of a buyer surface too, and never score their author", () => {
  ...
  // Tx 1: settles via the real escrow flow (confirmRelease auto-attaches the
  // buyer's satisfied review of the seller); the seller then reviews the
  // buyer through the same either-party pipeline.
  const tx1 = settleViaEscrow(db, buyer, seller, arbiter);
  tools.submitReview(db, {
    tx_id: tx1,
    reviewer_id: seller.agentId,
    outcome: "partial",
  ...
  const tx2 = settleViaEscrow(db, buyer, seller, arbiter);
  tools.submitReview(db, {
    tx_id: tx2,
    reviewer_id: seller.agentId,
    outcome: "satisfied",
